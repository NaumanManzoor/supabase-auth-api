...
2. Copy `.env.example` to `.env` and fill in your values:
   ...
   Find these in the Supabase Dashboard → **Project Settings → API Keys**. Use the **anon / publishable** key — never the `service_role` key.
...
- Only the Supabase **anon** key is used — the `service_role` key is never used.
